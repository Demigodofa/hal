/* HAL Web Relay JS (v20). Plain ASCII, no template literals. */
(function(){
  'use strict';
...
' : '') + parts.join(' ');
  ...
  // Wire inputs
  $('ghToken').value=S.ghToken; $('ghRepo').value=S.ghRepo; $('ghBranch').value=S.ghBranch; $('rememberSettings').checked=!!S.rememberSettings; $('rememberSsd').checked=!!S.rememberSsd; $('autoClear').value=S.autoClear||'1'; $('schedEnable').checked=!!S.schedEnable; $('schedEvery').value=String(S.schedEvery||30);
  ...
  function coerceGenesis(h){
    return (function(){ return Promise.resolve(); })().then(function(){
      var upper=(h && h.name ? h.name.toUpperCase() : '');
      if(upper==='GENESIS') return h;
  ...
  function setSsd(h){
    return coerceGenesis(h).then(function(g){
      ...
      $('ssdLabel').value = nm;
      if (ssdRoot && $('rememberSsd').checked){ return putHandle('ssdRoot', ssdRoot).catch(function(){}); }
    });
  }
  ...
  $('pickBtn').addEventListener('click', function(){
    try{
      ...
      if (!('showDirectoryPicker' in window)) {
        log('picker error: File System Access API not available (use Chrome/Edge).');
  ...
  function listDir(root,rel){ if(!rel||rel==='.') return Promise.resolve({path:'.',items:[]}).then(function(){ return root; }).then(function(){ return (async function(){ var dir=root; var items=[]; for await (var _i of dir.entries()){ var name=_i[0]; var handle=_i[1]; var item={name:name,is_dir:(handle.kind==='directory')}; if(handle.kind==='file'){ var f=await handle.getFile(); item.size=f.size; } items.push(item);} return {path:'.',items:items}; })(); }); return ensureDir(root,rel).then(function(dir){ return (async function(){ var items=[]; for await (var _i of dir.entries()){ var name=_i[0]; var handle=_i[1]; var item={name:name,is_dir:(handle.kind==='directory')}; if(handle.kind==='file'){ var f=await handle.getFile(); item.size=f.size; } items.push(item);} return {path:rel,items:items}; })(); }); }
...
'; var path='memory/short_term/checkpoints/short_term-'+slug()+'.json'; return ghPutFile({repo:repo, path:path, content:content, message:'chore: checkpoint '+payload.ts, branch:branch}).then(function(out){ log('checkpoint ok:', {path:path, commit:(out&&out.commit&&out.commit.sha?out.commit.sha.slice(0,7):'')}); return true; }); }).catch(function(e){ log('checkpoint error:', e && e.message ? e.message : String(e)); return false; }); }
  ...
  var schedTimer=null;
  function startScheduler(mins){ stopScheduler(); var ms=Math.max(1,parseInt(mins||30,10))*60*1000; schedTimer=setInterval(checkpointNow, ms); log('scheduler on:', String(ms/60000), 'min'); }
