(function(){
  ...
  function log(){ var out=[],i; for(i=0;i<arguments.length;i++){ out.push(toStr(arguments[i])); } var el=$('log'); el.textContent += (el.textContent?NL:'') + out.join(' '); el.scrollTop = el.scrollHeight; }
  ...
  function load(){ try{ var o=JSON.parse(localStorage.getItem(LS_KEY)||'{}'); var r={},k; for(k in DEF){ r[k]= (o.hasOwnProperty(k)? o[k] : DEF[k]); } return r; }catch(e){ return JSON.parse(JSON.stringify(DEF)); } }
    ...
    return Promise.resolve().then(function(){
      ...
      return h.getDirectoryHandle('GENESIS',{create:false}).then(function(d){ return d; }).catch(function(){
        return h.getFileHandle('.genesis_root',{create:false}).then(function(){ return h; }).catch(function(){
          return h.getFileHandle('.genesis_root',{create:true}).then(function(f){ return f.createWritable().then(function(w){ return w.write('ok'); if(w.close) try{w.close();}catch(_e){} }); }).then(function(){ return h; }).catch(function(){ return h; });
        });
      });
    });
  }
  ...
  function ensureDir(root,rel){ var parts=splitPath(rel); var d=Promise.resolve(root),i; for(i=0;i<parts.length;i++){ (function(part){ d=d.then(function(dir){ return dir.getDirectoryHandle(part,{create:true}); }); })(parts[i]); } return d; }
  function getParent(root,rel,create){ var parts=splitPath(rel); if(!parts.length) return Promise.reject(new Error('empty path')); var d=Promise.resolve(root),i; for(i=0;i<parts.length-1;i++){ (function(part){ d=d.then(function(dir){ return dir.getDirectoryHandle(part,{create:create}); }); })(parts[i]); } return d.then(function(dir){ return {dir:dir,name:parts[parts.length-1]}; }); }
  function writeFile(root,rel,content){ return getParent(root,rel,true).then(function(x){ return x.dir.getFileHandle(x.name,{create:true}).then(function(fh){ return fh.createWritable().then(function(w){ return w.write(content||''); if(w.close) try{w.close();}catch(_e){}; }).then(function(){ return {path:rel,bytes:(content||'').length}; }); }); }); }
  ...
  function appendJsonArray(root,rel,obj){ return readIfExists(root,rel).then(function(r){ var arr=[]; try{ arr=r.content?JSON.parse(r.content):[]; if(!Array.isArray(arr)) arr=[]; }catch(e){ arr=[]; } arr.push(obj); return writeFile(root,rel,JSON.stringify(arr,null,2)+NL); }); }
